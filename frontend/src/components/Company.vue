<template>
    ...
    <v-card style="width:450px;" outlined>
        ...
        <v-card-title v-if="value._links">
            Company # {{value._links.self.href.split("/")[value._links.self.href.split("/").length - 1]}}
        </v-card-title >
        <v-card-title v-else>
            Company
        </v-card-title >
    ...
    export default {
        ...
        methods: {
            ...
            async repairRequest() {
                try {
                    if(!this.offline) {
                        var temp = await axios.put(axios.fixUrl(this.value._links.repairrequest.href))
            ...
            async returnConfirm() {
                try {
                    if(!this.offline) {
                        var temp = await axios.put(axios.fixUrl(this.value._links.returnconfirm.href))
                        for(var k in temp.data) {
                            this.value[k]=temp.data[k];
                        }
                    }
